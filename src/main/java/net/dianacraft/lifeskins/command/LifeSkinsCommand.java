...
public class LifeSkinsCommand {
    ...
    public static void reloadSkin(ServerPlayerEntity player, int lives) throws CommandSyntaxException {
        SkinPathFinder spf = new SkinPathFinder(player);
        if (spf.hasSkins()) {
            SkinCommand.setSkin(player, () -> setSkinFromFile(spf.getSkinPath(lives), SkinPathFinder.getSlim(spf.getSkin(livesManager.getPlayerLives(player)))));  // TODO: [BUG] skins always set as classic, even if i hardcode true, seems to be a fabrictailor issue
        }
    }
