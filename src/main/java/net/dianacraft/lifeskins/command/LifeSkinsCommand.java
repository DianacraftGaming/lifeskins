...
import net.dianacraft.lifeskins.util.DummyRunnable;
...
public class LifeSkinsCommand {
    ...
    public static int reloadSkin(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (!currentSeries.hasAssignedLives(player)) {
    ...
    public static Runnable reloadSkin(ServerPlayerEntity player, int lives) throws CommandSyntaxException {
        SkinPathFinder spf = new SkinPathFinder(player);
        if (spf.hasSkins()) {
